...
// Galois multiplication of all bytes
// By 2
const GM2: [u8; 256] = [
...
pub fn mix_columns (
    ...
    for column in 0..4 {
        tmp[0] = GM2[array_in[column] as usize] ^ 
            GM3[array_in[column + 4] as usize] ^
            array_in[column + 8] ^ array_in[column + 12];

        tmp[1] = array_in[column] ^ 
            GM2[array_in[column + 4] as usize] ^
            GM3[array_in[column + 8] as usize] ^ 
            array_in[column + 12];

        tmp[2] = array_in[column] ^ array_in[column + 4] ^
            GM2[array_in[column + 8] as usize] ^ 
            GM3[array_in[column + 12] as usize];

        tmp[3] = GM3[array_in[column] as usize] ^ 
            array_in[column + 4] ^ array_in[column + 8] ^ 
            GM2[array_in[column + 12] as usize];
...
pub fn inverse_mix_columns (
    ...
    for column in 0..4 {
        tmp[0] = GM14[array_in[column] as usize] ^ 
            GM11[array_in[column + 4] as usize] ^ 
            GM13[array_in[column + 8] as usize] ^ 
            GM9[array_in[column + 12] as usize];

        tmp[1] = GM9[array_in[column] as usize] ^ 
            GM14[array_in[column + 4] as usize] ^ 
            GM11[array_in[column + 8] as usize] ^ 
            GM13[array_in[column + 12] as usize];

        tmp[2] = GM13[array_in[column] as usize] ^ 
            GM9[array_in[column + 4] as usize] ^ 
            GM14[array_in[column + 8] as usize] ^ 
            GM11[array_in[column + 12] as usize];

        tmp[3] = GM11[array_in[column] as usize] ^ 
            GM13[array_in[column + 4] as usize] ^ 
            GM9[array_in[column + 8] as usize] ^ 
            GM14[array_in[column + 12] as usize];
